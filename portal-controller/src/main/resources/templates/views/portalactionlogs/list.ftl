...
<script type="text/js-render" id="portalactionlogsTemplate">        <tr>
            <td>{{:id}}</td>
            <td>{{:operationType}}</td>
            <td>{{:parentCode}}</td>
            <td>{{:ip}}</td>
 ...
 <td> <span class="label label-info"><a href="/portalactionlogs/detail?id={{:id}}" onclick="">编辑</a></span> </td></tr>
</script>
<script type="text/javascript">    var portalactionlogsObject = {
    option: {
        pageIndex: 1,
        pageSize: 15,
        pageCount: 0,
        ...
        $("#btnSearch").click(function () {
            portalactionlogsObject.option.pageIndex = 1;
            portalactionlogsObject.loadData();
        });
    }, remove: function (id) {
        if (confirm('确认 改变角色状态吗？')) {
            $.post("/PortalActionLogs/changeStatues", {id: id}, function (data) {
                if (data.Code == 0) {
                    portalactionlogsObject.loadData();
                } else {
                    alert('操作失败!');
        ...
        $.post("/PortalActionLogs/changeAr", {id: id, ar: obj}, function (data) {
            if (data.Code == 0) {
                portalactionlogsObject.loadData();
            }
        });
        ...
        $("#gvResult").html(portalactionlogsObject.option.loading);
        $.getJSON("/PortalActionLogs/getPageList", {
            pageIndex: portalactionlogsObject.option.pageIndex,
            pageSize: portalactionlogsObject.option.pageSize,
            key: ""
        }, function (data) {
            var obj = data.data, content = [];
            $("#gvResult").html(portalactionlogsObject.option.listHeader);
            portalactionlogsObject.option.count = obj.count;
            for (var i = 0; i < obj.items.length; i++) {
            }
            $("#gvResult").append($("#portalactionlogsTemplate").render(obj.items));
            $.each($('.isAr'), function () {
                new Medium({
            ...
            $('.isAr').on('blur', function () {
                portalactionlogsObject.changeAr($(this).attr('data-id'), $(this).html());
            });
            App.handleUniform();
            <!--pager-->
            var pager = new pagination.init('Pager');
            pager.itemCount = portalactionlogsObject.option.count;
            pager.size = portalactionlogsObject.option.pageSize;
            pager.index = portalactionlogsObject.option.pageIndex;
            pager.onclick = function (indexs) {
                portalactionlogsObject.option.pageIndex = indexs;
                portalactionlogsObject.loadData();
            };
            pager.render();
