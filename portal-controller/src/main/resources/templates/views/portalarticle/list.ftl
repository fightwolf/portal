            ...
            <div class="row-fluid">
                <div class="span12"><h3 class="page-title"></h3>
                    <ul class="breadcrumb">
                        ...
                        <li><a href="/portalarticle/list">列表</a> <i class="icon-angle-right"></i></li>
                    </ul>
                </div>
            </div>
            <div class="row-fluid">
...
<script type="text/js-render" id="portalarticleTemplate">        <tr>
            <td>{{:id}}</td>
            <td>{{:title}}</td>
<td><img src="{{:icon}}" alt="" style="height:50px;" class="plus-cursor" data-image="{{:icon}}" data-title="" data-caption="" /></td>
 ...
 <td> <span class="label label-info"><a href="/portalarticle/detail?id={{:id}}" onclick="">编辑</a></span> </td></tr>
</script>
<script type="text/javascript">    var portalarticleObject = {
    option: {
        pageIndex: 1,
        pageSize: 15,
        pageCount: 0,
        ...
        $("#btnSearch").click(function () {
            portalarticleObject.option.pageIndex = 1;
            portalarticleObject.loadData();
        });
    }, remove: function (id) {
        if (confirm('确认 改变角色状态吗？')) {
            $.post("/PortalArticle/changeStatues", {id: id}, function (data) {
                if (data.Code == 0) {
                    portalarticleObject.loadData();
                } else {
                    alert('操作失败!');
        ...
        $.post("/PortalArticle/changeAr", {id: id, ar: obj}, function (data) {
            if (data.Code == 0) {
                portalarticleObject.loadData();
            }
        });
        ...
        $("#gvResult").html(portalarticleObject.option.loading);
        $.getJSON("/PortalArticle/getPageList", {
            pageIndex: portalarticleObject.option.pageIndex,
            pageSize: portalarticleObject.option.pageSize,
            key: ""
        }, function (data) {
            var obj = data.data, content = [];
            $("#gvResult").html(portalarticleObject.option.listHeader);
            portalarticleObject.option.count = obj.count;
            for (var i = 0; i < obj.items.length; i++) {
            }
            $("#gvResult").append($("#portalarticleTemplate").render(obj.items));
            $.each($('.isAr'), function () {
                new Medium({
            ...
            $('.isAr').on('blur', function () {
                portalarticleObject.changeAr($(this).attr('data-id'), $(this).html());
            });
            App.handleUniform();
            <!--pager-->
            var pager = new pagination.init('Pager');
            pager.itemCount = portalarticleObject.option.count;
            pager.size = portalarticleObject.option.pageSize;
            pager.index = portalarticleObject.option.pageIndex;
            pager.onclick = function (indexs) {
                portalarticleObject.option.pageIndex = indexs;
                portalarticleObject.loadData();
            };
            pager.render();
