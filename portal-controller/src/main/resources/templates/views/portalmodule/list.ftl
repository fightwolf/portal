            ...
            <div class="row-fluid">
                <div class="span12"><h3 class="page-title"></h3>
                    <ul class="breadcrumb">
                        ...
                        <li><a href="/portalmodule/list">列表</a> <i class="icon-angle-right"></i></li>
                    </ul>
                </div>
            </div>
            <div class="row-fluid">
...
<script type="text/js-render" id="portalmoduleTemplate">        <tr>
            <td>{{:id}}</td>
            <td>{{:moduleName}}</td>
            <td>{{:moduleCode}}</td>
<td><img src="{{:icon}}" alt="" style="height:50px;" class="plus-cursor" data-image="{{:icon}}" data-title="" data-caption="" /></td>
 ...
 <td> <span class="label label-info"><a href="/portalmodule/detail?id={{:id}}" onclick="">编辑</a></span> </td></tr>
</script>
<script type="text/javascript">    var portalmoduleObject = {
    option: {
        pageIndex: 1,
        pageSize: 15,
        pageCount: 0,
        ...
        $("#btnSearch").click(function () {
            portalmoduleObject.option.pageIndex = 1;
            portalmoduleObject.loadData();
        });
    }, remove: function (id) {
        if (confirm('确认 改变角色状态吗？')) {
            $.post("/PortalModule/changeStatues", {id: id}, function (data) {
                if (data.Code == 0) {
                    portalmoduleObject.loadData();
                } else {
                    alert('操作失败!');
        ...
        $.post("/PortalModule/changeAr", {id: id, ar: obj}, function (data) {
            if (data.Code == 0) {
                portalmoduleObject.loadData();
            }
        });
        ...
        $("#gvResult").html(portalmoduleObject.option.loading);
        $.getJSON("/PortalModule/getPageList", {
            pageIndex: portalmoduleObject.option.pageIndex,
            pageSize: portalmoduleObject.option.pageSize,
            key: ""
        }, function (data) {
            var obj = data.data, content = [];
            $("#gvResult").html(portalmoduleObject.option.listHeader);
            portalmoduleObject.option.count = obj.count;
            for (var i = 0; i < obj.items.length; i++) {
            }
            $("#gvResult").append($("#portalmoduleTemplate").render(obj.items));
            $.each($('.isAr'), function () {
                new Medium({
            ...
            $('.isAr').on('blur', function () {
                portalmoduleObject.changeAr($(this).attr('data-id'), $(this).html());
            });
            App.handleUniform();
            <!--pager-->
            var pager = new pagination.init('Pager');
            pager.itemCount = portalmoduleObject.option.count;
            pager.size = portalmoduleObject.option.pageSize;
            pager.index = portalmoduleObject.option.pageIndex;
            pager.onclick = function (indexs) {
                portalmoduleObject.option.pageIndex = indexs;
                portalmoduleObject.loadData();
            };
            pager.render();
